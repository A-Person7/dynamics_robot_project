                ...
                \begin{axis}[
                    ...
                    \addplot3[domain = 0:0.37095212, no marks, surf]
                        (
                            ...
                            {sqrt(0.25 + f(x)^2) * (2*g(x)*(4*f(x)^2+1)-16*f(x)*g(x)^2)/(4*f(x)^2+1)^2 + 2 *f(x)*g(x)/sqrt(0.25 + f(x)^2)* 2*g(x)/(4*f(x)^2+1)},
                            {x}
                        );
                \end{axis}
            \end{tikzpicture}

            \caption{Radial and tangential components of acceleration (meters per second squared) over time (seconds)}\label{actuatorAccel}
        \end{figure}
